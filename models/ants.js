...
class AntsModel extends Model {
  ...
  setup () {
    ...
    this.foodColor = 'blue'
    this.nestColorMap = ColorMap.gradientColorMap(20, ['black', this.nestColor])
    this.foodColorMap = ColorMap.gradientColorMap(20, ['black', this.foodColor])
  ...
  updateTurtles () {
    this.turtles.ask(t => {
      if (t.id >= this.anim.ticks) return // slowly release ants
      this.wiggleUphill(t)
  ...
  updatePatches () {
    ...
    this.patches.ask(p => {
      ...
      if (!p.isNest && !p.isFood) {
        p.color = p.foodPheromone > p.nestPheromone
...
const options = Model.defaultWorld(40)
const model = new AntsModel(document.body, options)
model.setup()
model.start()

// Debugging
util.toWindow({ ColorMap, Model, util })
const { patches, turtles, links, world } = model
util.toWindow({ patches, turtles, links, world, model })
