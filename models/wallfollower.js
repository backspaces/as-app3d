...
class WallFollowerModel extends Model {
  setup () {
    ...
    this.righty.setDefault('color', 'red')
      // .setDefault('size', 2)
      .setDefault('turn', -1) // -90 degrees
      .setDefault('atEdge', 'wrap')
    this.lefty.setDefault('color', 'green')
      // .setDefault('size', 2)
      .setDefault('turn', +1) // 90 degrees
      .setDefault('atEdge', 'wrap')
    ...
    this.patches.ask(p => {
      if (util.randomFloat(1) < 0.04) {
        ...
        neighbors.ask(n => { n.setBreed(this.walls) })
      }
    })
    // Make sure single patch gaps are filled.
    this.patches.edgePatches().ask(p => { p.setBreed(this.patches) })
    this.patches
    ...
    this.walls.ask(p => { p.color = [222, 184, 135] })

    // Sprout this.population agents on the non-wall patches.
    this.patches
      .filter(p => !p.isBreed(this.walls))
      .nOf(this.population).ask(p => {
  ...
  step () {
    this.turtles.ask(t => { this.walk(t) })
  }
  walk (turtle) {
...
util.toWindow({ p: patches.oneOf(), t: turtles.oneOf(), w: walls.oneOf(), model, util })
