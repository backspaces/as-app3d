import {Color, ColorMap, Model,  util } from '../dist/as-app3d.esm.min.js'
...
class FireModel extends Model {
    setup() {
        ...
        this.burnedTrees = 0
        this.initialTrees = this.patches.filter(p =>
            p.color.equals(this.treeColor)
        ).length
    }
    ...
    fadeEmbers() {
        this.embers.ask(p => {
            ...
            if (ix === this.fireColorMap.length - 1) p.setBreed(this.patches)
